...
use super::{
    hypersync::{ClientConfig, HypersyncClient},
    kafka::{config::KafkaConfig, producer::KafkaProducer},
};
...
impl DataCollectionService {
    pub fn new(client_config: ClientConfig, kafka_config: KafkaConfig) -> Result<Self> {
        let client = HypersyncClient::new(client_config)?;
    ...
    pub async fn collect_historical_transactions(
        ...
        let to_txs = self
            .client
            .query_transactions_to_address(address.clone(), from_block, to_block) //TODO: consider removing clone
            .await
    ...
    pub async fn start_real_time_monitoring(&self, address: &str) -> Result<()> {
        ...
        // Implementation for real-time monitoring will typically:
        ...
        for tx_batch in response.data.transactions {
            for tx in tx_batch {
                ...
                let event = TransactionEvent {
                    ...
                    block_number: tx.block_number.unwrap_or_default().into(),
